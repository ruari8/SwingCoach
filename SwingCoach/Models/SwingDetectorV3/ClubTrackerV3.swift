...
nonisolated struct ClubEvidenceV3: Equatable {
...
nonisolated final class ClubTrackerV3 {
    ...
    func evidence(
        ...
        guard let lock, !window.isEmpty else { return ClubEvidenceV3() }
        ...
        for frame in window {
            ...
            let heldClub = frame.heldClub
            if let heldClub {
                points.append((frame.realTime, heldClub.point))
            }

            var frameNear = 0.0
            for box in heldClub?.boxes ?? [] where box.confidence >= 0.25 {
                let distance = GeometryV3.distance(between: box.rect, and: lock.patchRect)
                let proximity = GeometryV3.inverseRamp(distance, low: 0.0, high: 0.16)
                let confidence = GeometryV3.ramp(box.confidence, low: 0.25, high: 0.82)
        ...
        if points.count >= 2 {
            for index in 1..<points.count {
                ...
                let speed = GeometryV3.distance(previous.point, current.point) / dt
                let speedScore = GeometryV3.ramp(speed, low: 0.05, high: 0.34)
                let nearPatch = max(
                    GeometryV3.inverseRamp(GeometryV3.distance(previous.point, lock.ballCenter), low: 0.0, high: 0.22),
                    GeometryV3.inverseRamp(GeometryV3.distance(current.point, lock.ballCenter), low: 0.0, high: 0.22)
                )
        ...
        let pathScore = GeometryV3.ramp(pathSpan, low: 0.06, high: 0.36)
        let verticalScore = GeometryV3.ramp(verticalSpan, low: 0.08, high: 0.34)
        ...
        let lumaScore = GeometryV3.ramp(maxLumaMotion, low: 0.006, high: 0.050)
        let clubCoverage = Double(clubFrameCount) / Double(max(1, window.count))
        let coverageScore = GeometryV3.ramp(clubCoverage, low: 0.05, high: 0.30)
    ...
    private static func nearAwayNearSequence(
        ...
        let samples = points.map { sample in
            let distance = GeometryV3.distance(sample.point, ballCenter)
            return (
                near: GeometryV3.inverseRamp(distance, low: 0.0, high: 0.22),
                away: GeometryV3.ramp(distance, low: 0.16, high: 0.46)
            )
        }
