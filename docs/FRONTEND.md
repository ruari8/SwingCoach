...
- V3 test metadata lives in `tools/detector_test_v3_labels.json`. Impact labels are rough one-second source-timeline buckets from QuickTime review; slow-motion real-time equivalents are source seconds divided by each video's `source_time_scale`. The V3 performance runner evaluates clips shortest-to-longest by actual video duration, which keeps iteration fast on the small fixtures before the long range sessions. Run `python3 tools/evaluate_detector_test_v3_performance.py --reuse-reports` to rescore existing V3 detector outputs against the current labels without rerunning Core ML.
- In the narrowed V3 iteration, `test2` is the active positive acceptance clip because the deployed YOLO model sees the addressed white ball and club. `test1` and `test3` are temporarily siloed as yellow-ball object-model coverage cases, and `test6` is treated as a low-quality screen-recording/UI-contamination edge case rather than a v1 acceptance clip.
- The impact-centered detector now treats ball departure plus club contact as the primary proof of impact, with local motion retained as a softer sanity check. This lets `test2` produce one hybrid impact at `2.333s` with a `0.20s` declaration delay when run at `16fps`, source scale `1`, detector timeline scale `8`, and CPU/Neural Engine compute.
- The club-contact gate is two-tiered: normal impacts still need tight club/ball overlap, but very clean ball departures can accept a looser club-box distance up to `0.12` normalized frame units. This fixes the second `test5` labelled window on the local proxy, where the ball disappears cleanly but the nearest detected club box sits about `0.102` from the ball anchor.
