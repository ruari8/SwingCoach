...
- Orientation policy: upright portrait only throughout Capture, Library, Trim, review/player, Coach, and Debug. Both Debug and Release declare only `UIInterfaceOrientationPortrait` for iPhone and iPad. iPad requests full-screen presentation for this fixed-orientation UI. There is no rotation toggle. This interface policy does not change camera capture angles or recorded-video track transforms.
...
- Local-only clips support Library and Auto review playback, deletion, batch sharing, and analysis upload preparation. Photos validation preserves them. Library loads local thumbnails before checking Photos access, so saved previews return after relaunch even when Photos is denied. Auto review deletes a local-only clip without Photos permission and labels the action “Delete from SwingCoach”. Local originals participate in device backup; Photos-backed copies and bundled references remain excluded per file.
- Both Trim range export and **Use Full Video** obey the preference. With ON, using an entire imported Photos video reuses that asset as before. With OFF, it stores a local copy and leaves the imported original in Photos. A partial save leaves Trim open with an error and only the failed clips available for retry, avoiding duplicate saves. Analysis receives only the selected clips that saved successfully.
...
- Capture uses an edge-to-edge camera preview behind the native tab bar, with dark appearance for readable controls over the preview. The top row contains the frame-rate menu and native `Auto / Manual` selector, with Auto first. Auto shows a compact bottom-left status with a red active dot and a bottom-right thumbnail/saved-count shortcut. The shortcut is disabled for an empty session. Saving and permission/detector failures remain visible in the status instead of a setup banner. Tapping the saved count opens a full-screen paged video carousel with a `Done` button, starting on the latest saved swing on every opening. Session order stays chronological; swipe right to reach older swings. Returning after another capture opens that new final clip. Opening review stops the capture session entirely (no camera indicator, no encode/inference load, no new detections mid-review); dismissing it restarts the session and resets the live detector/buffer timeline. Auto review and Library use the shared `SwingReviewPager` with native scroll snapping and a clipping boundary around each video. Only the selected Auto review video can play, and only the selected page accepts player input. Deleting the selected clip opens its next neighbour, or the previous clip at the end of the session. The carousel supports playback and permanent deletion from both SwingCoach and Photos, with the delete control in the player's bottom-right accessory slot so it never overlaps the player's own corner controls.
- Auto review keeps its left-edge pencil rail outside the moving pages, so it stays fixed during swipes. The `Draw straight lines` button opens the same yellow full-swing line canvas as Library. Entering drawing mode pauses playback and paging. `Finish drawing lines` restores review gestures; `Undo last line` and `Clear all lines` edit the selected clip. Drawings save immediately to `manual_annotations.json` under the existing `swing-<SavedSwing UUID>` key, so the same clip retains them when reopened in Auto review or Library, including after app relaunch. Lines remain local overlays and are not burned into exported videos. Analysis-result drawings remain attached to their analysis result separately. Related memory clarification: [#32](https://github.com/ruari8/SwingCoach/issues/32).
...
- Library > gear > Experiments > **Extra swing footage** provides independent **Before swing** and **After swing** controls. Each defaults to 0 and accepts 0–30 real seconds, using half-second +/− steps or a typed value. The preference persists and affects newly detected Auto clips and Manual/Trim ranges; it does not change detection thresholds, existing saved clips, or hand-marked ranges. It replaces Trim's former fixed 1-second padding.
- Auto snapshots the extra footage when a detection is accepted, reports it as pending while collecting the trailing frames, and saves available footage if capture stops early. The rolling buffer still encodes 20-second chunks with 2.6 seconds of overlap. Longer clips join non-overlapping portions of those files in memory before the existing single export pass; they do not enlarge the continuously encoded overlap. Dropped-frame gaps between files retain their source-time positions, so later footage and detection timestamps stay aligned; a clip ending inside a gap stops at its last available frame. Normal buffer retention stays at 45 seconds; pending requests protect any older ranges they still need, and files used by an export stay pinned until it completes. A new buffer session cannot select old-session chunks. Keep camera orientation steady across the requested clip: a mid-clip rotation reports an error instead of exporting a partly rotated video.
- Auto capture review belongs to the session that accepted each detection. Starting another session clears its review and counters; unfinished clips from the previous session still save to Library without appearing in the new review or changing its pending count.
...
- Run `./scripts/verify-capture-controls.sh` for the Capture layout, Auto-first ordering, model-stats default/persistence/on-off behavior in both modes, disabled/unavailable/waiting/error states, portrait controls after left/right/upside-down device rotation across tabs, Trim and review, saved review playback/paging/deletion, and Manual → Trim → Cancel. It drives production UI with XCUITest in a scratch build, substituting deterministic detector snapshots and synthetic camera file output. Review uses bundled reference clips and deletion only removes fixture entries. Real camera preview orientation, recorded-video metadata after physical rotation, detector performance, and Photos deletion remain device checks. The Simulator pass checks the built orientation declarations for both device families and drives the iPhone UI; it does not verify iPad windowing.
...
- Detected Trim ranges use the **Extra swing footage** settings captured when Trim opens, clamped to the available source. Imported slow-motion timelines convert the real-second extensions using the detector's source-time factor. Hand-marked ranges are unchanged.
- Trim keeps its slow-motion playback and displayed time scale for fresh high-FPS captures. Its periodic observer interval scales with the preview rate to refresh every 0.1 playback seconds: at 240 fps, observe every 0.0125 source seconds at ⅛ speed. This prevents the former 0.8-second timer jumps without changing playback speed or export timing. Already-retimed imports use their existing source timeline.
...
- `SWINGCOACH_TRIM_VERIFY_CONTEXT=1 ./scripts/verify-trim-review.sh <artifact-directory>` checks setting persistence, asymmetric padding, the 240 fps preview timecode, real encoded buffer chunks spanning multiple files, early-stop completion, monotonic frame timestamps with no overlap duplication, and exported audio/video duration. The normal Trim driver explicitly seeds its historical 1-second padding fixture.
- `./scripts/verify-trim-playback-clock.sh` uses a generated 240 fps video and real AVPlayer at ⅛ speed on macOS to reproduce the old 0.8-second jumps and verify the production observer refreshes in 0.1-second steps.
...
- Lock pins playback controls across next and previous clips in both Library and Auto review. Unlock applies to the entire open review and restores tap-to-hide and playback auto-hide. Each review presentation starts unlocked. Auto places delete above the timeline so it remains separate from Lock. `SwingDetailView` and `AutoSwingReviewView` own the preference and pass `controlsLocked: Binding<Bool>` to every player; omitting the binding hides Lock in standalone players.
...
- The drawing tool loads the video track's dimensions and orientation before becoming available. Lines follow the visible video when resizing its viewport, and strokes beginning in the letterbox are ignored. Lines saved before this correction may need clearing and redrawing once because the old format did not record the viewport needed to repair their coordinates.

Review regression checks:
- Run `scripts/verify-review-lock.sh` to test Lock and explicit unlock across forward and backward paging in Library and Auto. The test also checks pinned controls during playback, restored tap-to-hide after unlocking, and a fresh unlocked Library review. It uses three synthetic videos in a disposable Simulator, saves doctor output, XCTest results, and screenshots under `.verification-artifacts/review-lock/`, and deletes only its own Simulator. Auto uses the production review with the DEBUG session fixture; this does not verify live capture or Photos on a phone.
- Run `scripts/verify-review.sh` on macOS with Xcode and FFmpeg. It creates synthetic media and a disposable Simulator, tests per-video analysis ownership and retries, verifies oriented video dimensions, and drives Library filtering, starred order, reference navigation, and drawing while the device rotates. It preserves logs, XCTest results and screenshots in `.verification-artifacts/review-fixes/` and removes its Simulator afterward. It then verifies bulk deletion and relaunch persistence, including preserved reference entries and local-file cleanup. No phone, Photos originals, private reference assets, backend or upload is needed.
...
Run `scripts/verify-save-photos.sh` for an isolated Simulator check. It substitutes camera video and detector windows in a scratch build, then uses real Auto and Trim exporters, app persistence, and PhotoKit. The OFF phase runs with Photos denied and checks local playback after relaunch, Auto review deletion, and no new Photos items. The ON phase grants Simulator Photos access and checks one new Photos item for each Manual full-video, Trim range, and Auto export. The script snapshots existing Photos asset IDs before testing and compares newly created IDs with saved library records; a fresh Simulator can already contain seeded assets. Unit tests cover local copy failure, asset validation, thumbnails, playback, deletion, and analysis upload preparation. To reuse passing OFF evidence and run only ON, pass an artifact directory followed by `--on-only`. Evidence stays under `.verification-artifacts/save-photos/`. Real camera capture and physical-iPhone Photos interoperability still require a device check.
