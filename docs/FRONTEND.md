...
- `./scripts/verify-trim-review.sh` verifies a long Manual → Trim session, late-clip seeking, full-screen paging/playback, export-only with analysis selected, and chosen-subset handoff after relaunch. It creates an isolated Simulator, synthetic camera/detector inputs and real Simulator Photos exports, and records the Coach boundary without contacting the backend. Unit checks cover padding/clamping, source-time scaling, saved-ID mapping and review duration/orientation. Real range camera input, device Photos behavior and live analysis remain device/service checks.
