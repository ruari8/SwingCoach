...
- Auto snapshots the extra footage when a detection is accepted, reports it as pending while collecting the trailing frames, and saves available footage if capture stops early. The rolling buffer still encodes 20-second chunks with 2.6 seconds of overlap. Longer clips join non-overlapping portions of those files in memory before the existing single export pass; they do not enlarge the continuously encoded overlap. Normal buffer retention stays at 45 seconds; pending requests protect any older ranges they still need, and files used by an export stay pinned until it completes. A new buffer session cannot select old-session chunks. Keep camera orientation steady across the requested clip: a mid-clip rotation reports an error instead of exporting a partly rotated video.
