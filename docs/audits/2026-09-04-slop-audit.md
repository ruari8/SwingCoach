...
> Historical findings from 4 September. The cleanup for [issue #15](https://github.com/ruari8/SwingCoach/issues/15) implements these removals and strengthens the tests. Source links below point to the audited revision so deleted code remains inspectable. The current `slop_evidence.py --probe-tests` checks that the repaired tests reject faults.
