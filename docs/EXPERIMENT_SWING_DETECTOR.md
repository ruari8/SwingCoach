...
Historical interpretation from the pre-`SwingDetectorV2` app path:
...
Live Capture previously switched between `contact`, `impact`, and `hybrid` from Library > Experiments. That mode switch has been removed from the app; this paragraph is retained only to explain older fixture/debug reports that mention those names.
...
- Training clip `swingcoach_050_dtl_20260321_220625.mp4` exposed an over-tight horizontal foreground-ball gate: raw YOLO saw the addressed ball near normalized `x ~= 0.90`, but live impact logic ignored it because foreground balls were capped at `x <= 0.84`. The right-side cap is relaxed so this valid strike-area ball can become an anchor. With the duration rule used for training-data spot checks (`<10s => 4x`, `>=10s => 8x`), clips `010`, `020`, `030`, `040`, and `050` each produce one hybrid swing after the change.
