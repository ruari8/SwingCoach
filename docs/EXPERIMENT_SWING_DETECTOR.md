...
- At `8fps`, `240/8x` source timing, `0.20s` confirmation, and CPU/Neural Engine compute, focused `test5` windows around swings 2 and 4 still show clear addressed-ball disappearance and pass the shared hybrid selector at about `764s` and `1723s`. On the current full proxy, raw impact returns about `764s`, `1244s`, `1488s`, and `1723s`; hybrid keeps `764s`, `1244s`, and `1723s`, rejecting the `1488s` setup/false-positive window through the pose/body-drift gate. This means the `8fps` failure mode is not that impact is invisible, but that lower source sampling can change which long-session impacts survive the full stream.
