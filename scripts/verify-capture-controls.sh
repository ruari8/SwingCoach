...
cp "$repo/scripts/verification/ManualTrimCancelUITests.swift" "$repo/scripts/verification/CaptureControlsUITests.swift" "$scratch/repo/SwingCoachUITests/"
...
xcrun simctl bootstatus "$device" -b
{
    ...
    echo "actions=Auto/Manual layout,settings persistence,stats on/off,record/stop/Trim/Cancel,disabled/unavailable/waiting/error states,review playback/paging/deletion"
...
xcodebuild -project "$scratch/repo/SwingCoach.xcodeproj" -scheme SwingCoach -configuration Debug \
    ...
    -only-testing:SwingCoachUITests/CaptureControlsUITests \
...
print('PASS: capture controls, model stats persistence, recording/Trim return and shared review')
PY
