...
- Frontend capture/library/trim flows are implemented, including experimental `SwingDetectorV2` YOLO/Core ML preselection of likely swing clips during manual capture and editable preselected clips in the trim editor. Capture also has an Auto workflow that arms immediately on the Capture tab, keeps an overlapping rolling video buffer, and saves accepted detector swing clips directly to the library without opening Trim. Capture, imported Trim detection, and DEBUG Replay Debug now use the same V2 detector core with configurable real-time low sample rates and V2-owned adaptive burst sampling.
